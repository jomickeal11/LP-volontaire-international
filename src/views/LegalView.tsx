          ...
          <ol className="flex items-center gap-2 text-xs text-[#7A8A9A]">
            <li>
              <button
                ...
                className="hover:text-[#174F7A] transition-colors cursor-pointer"
              >
                {homeLabel}
              </button>
            </li>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <aside className="hidden lg:block lg:col-span-4 xl:col-span-3 sticky top-28 space-y-6">
                    ...
                    return (
                      <a
                        ...
                        className={`group flex items-start gap-2 py-1.5 px-2.5 rounded text-xs transition-colors ${
                          isActive
                            ? "text-[#174F7A] font-semibold bg-[#F1F5F9]"
                            : "text-[#4A5A6A] hover:text-[#174F7A] hover:bg-[#F8FAFC]"
                        }`}
                      >
                  ...
                  {otherDocs.map((item) => {
                    ...
                    return (
                      <Link
                        key={item.type}
                        href={`/${langKey}/${slug}`}
                        className="block text-xs text-[#4A5A6A] hover:text-[#174F7A] hover:underline py-1 transition-colors"
                      >
                        {item.label}
                      </Link>
                    )
            ...
            <div className="px-1">
              <button
                onClick={() => navigate("home")}
                className="text-xs text-[#7A8A9A] hover:text-[#174F7A] transition-colors cursor-pointer"
              >
                {backHomeLabel}
              </button>
            </div>
          ...
          <article className="col-span-1 lg:col-span-8 xl:col-span-9 bg-white rounded-xl border border-[#E2E8F0] p-6 sm:p-10 lg:p-14">
            ...
            <header className="mb-8 sm:mb-10">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#174F7A] tracking-tight uppercase mb-2 leading-tight">
                {doc.title}
              </h1>
              ...
              {doc.sections && doc.sections.length > 0 && (
                <div className="lg:hidden mb-6">
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] text-xs font-semibold text-[#174F7A] hover:bg-[#F1F5F9] transition-colors cursor-pointer"
                  >
                    ...
                    <nav className="mt-2 p-3 rounded-lg border border-[#E2E8F0] bg-white space-y-1 text-xs">
                      {doc.sections.map((s) => (
                        <a
                          ...
                          className="flex items-center gap-2 px-2 py-1.5 rounded text-[#4A5A6A] hover:text-[#174F7A] hover:bg-[#F8FAFC] transition-colors"
                        >
            ...
            <div className="space-y-10 sm:space-y-12">
              {doc.sections.map((section, index) => (
                ...
                >
                  <h2 className="text-lg sm:text-xl font-semibold text-[#174F7A] tracking-tight mb-3 sm:mb-4">
            ...
            <footer className="mt-12 sm:mt-16 pt-8 border-t border-[#E2E8F0]">
              <h3 className="text-sm font-semibold text-[#174F7A] mb-2">
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs sm:text-sm text-[#1A2B3C]">
                <a
                  ...
                  className="text-[#174F7A] hover:underline font-medium"
                >
              ...
              {otherDocs.map((item) => {
                ...
                return (
                  <Link
                    key={item.type}
                    href={`/${langKey}/${slug}`}
                    className="text-[#174F7A] hover:underline"
                  >
                    {item.label}
                  </Link>
                )
